...
import { FindNodes, SaveMenu, UpdateSysMenuById, RemoveSysMenuById } from '@/api/menu/index'
...
const fetchData = async () => {
  const result: any = await FindNodes()
...
const remove = async (id: number) => {
  ...
  ElMessageBox.confirm('此操作将永久删除该记录, 是否继续?', 'Warning', {
    ...
    const resutl: any = await RemoveSysMenuById(id)
    if (resutl.code === 200) {
      ElMessage.success('删除成功')
      fetchData()
    } else {
      ElMessage.error(resutl.message)
    }
  })
}
</script>
